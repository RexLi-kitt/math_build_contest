"""顺序变体双场景筛选: 正常100(seed 20260928) + 压力100(seed 20260929)。

策略: Base / OrdArea(纯覆盖) / MixA1 / MixA3(边界加权覆盖)。
两场景都赢才值得上全量验证。
"""
...
from cert_order_agents import MixA1Agent, MixA3Agent, OrderAreaAgent  # noqa: E402
...
def run_stage(cases, seed, label):
    ...
    for name in STRATS:
        ...
        print(f"{name:>8}: {sum(r['all_cleared'] for r in g)}/{len(g)} "
              f"mean={statistics.mean(per):7.2f} p95={q4_optimize.percentile(per, 0.95):7.2f} "
              f"det={statistics.mean(r['detection_time_s']/r['source_count'] for r in g):6.1f} "
              f"meas={statistics.mean(r['measure_actions'] for r in g):6.1f}", flush=True)
