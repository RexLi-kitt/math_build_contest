"""顺序变体验证: 正常500(seed 20260923) + 边界压力200(seed 20260924)。

策略: Base(原TSP顺序) / OrdArea / OrdDens。协议与 run_validation.py 一致。
"""
...
def summarize(rows, output, note):
    ...
    for s in summary:
        print(f"{s['strategy']:>9}: {s['all_clear']}/{s['cases']} "
              ...
              f"sw={s['mean_switching_s']:5.1f} "
              f"meas={s['mean_measure_actions']:6.1f}", flush=True)
    return summary
...
def main():
    ...
    summarize(rows, HERE / "results" / "order_holdout500",
              f"normal 500 cases seed={seed}")
    ...
    summarize(rows, HERE / "results" / "order_stress200",
