"""联合优化验证: 正常500(seed 20260923) + 压力200(seed 20260924)。

策略: Bplus(现行) / AggOrder(29站+覆盖顺序) / BNewOrder(27站+新顺序)。
"""
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=jobs) as pool:
        for done, row in enumerate(pool.map(q4_optimize._run_work, work), 1):
            rows.append(row)
            if done % 375 == 0 or done == len(work):
                print(f"normal {done}/{len(work)}", flush=True)
    summarize(rows, HERE / "results" / "joint_holdout500",
    ...
    summarize(rows, HERE / "results" / "joint_stress200",
