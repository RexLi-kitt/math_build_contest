...
第一轮固定种子 20260911 的结果：100/100 个随机组通过且均为凸多边形，9/9 项边界测试通过，最后的等边三角形反例通过。该结果只说明这些测试未发现错误，不能代替数学证明，也不表示任意输入均可成功定位。
...
默认工作簿位于 `outputs/q1/tables/Q1_定位实验.xlsx`，包含“定位实验”“观测明细”“边界测试与方法”。预览与导出检查日志放入本次运行独有的系统临时目录，结束后自动清理；如需人工查看预览，可设置环境变量 `Q1_KEEP_PREVIEWS=1` 保留。表格是运行结果快照，更改坐标后必须重跑实验和导表，不会在 Excel 内自动调用几何函数。当前未开展误差界敏感性实验。
