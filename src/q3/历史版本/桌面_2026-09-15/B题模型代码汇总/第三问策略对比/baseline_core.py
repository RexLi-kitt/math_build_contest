...
# 最坏覆盖闭式 sqrt(1800^2+R^2-2*1800*R*cos30°),R=1150 时为 988.5 m,
# 故模型 I 采用 1150 m 仍满足保证;该配置对旋转角不变。
...
class SevenPointAgent:
    ...
    def __init__(self, sim: OfflineSimulator, verbose: bool = False):
        ...
        self.search_points = [(0.0, 0.0)] + [
            add((0.0, 0.0), self.search_ring_r, 60.0 * k) for k in range(6)
        ]

    def _set_ring_rotation(self, theta_deg: float) -> None:
        """Re-place the six ring sites; coverage guarantee is rotation invariant."""
        self.search_points = [(0.0, 0.0)] + [
            add((0.0, 0.0), self.search_ring_r, theta_deg + 60.0 * k)
            for k in range(6)
        ]
    ...
    def search(self) -> None:
        ...
        for index, point in enumerate(self.search_points):
            ...
            next_point = self.search_points[index + 1] if index + 1 < len(self.search_points) else None
            self._opportunistic_clear(next_point)
    ...
    def _try_clear(self, point: Point, channel: int) -> bool:
        """同一静止目标在同一位置清除失败后，不再重复该动作。"""
        st = self.state[channel]
        if any(dist(point, previous) <= 1e-6 for previous in st.failed_clear_points):
            return False
        success = self.sim.clear(point, channel)
        if not success:
            st.failed_clear_points.append(point)
        return success
    ...
    def _clear_neighborhood(self, estimate: Point, channel: int,
        ...
        for dx, dy in offsets:
            if self._try_clear((estimate[0] + dx, estimate[1] + dy), channel):
                return True
        return False
    ...
    def _opportunistic_clear(self, next_point: Optional[Point]) -> None:
        """按插入下一搜索点之前的额外路程选择目标，绕路预算为 250 m。

        每次清除后重新计算；最后一个搜索点交由统一路线调度处理。
        预算包含中心清除失败后十字兜底的路径，避免低估绕路。
        """
        ...
        while True:
            ...
            for channel, st in self.state.items():
                ...
                if estimate is None or math.hypot(*estimate) > AREA_R * 1.2:
                    continue
                path = [self.sim.position, estimate] + [
                    (estimate[0] + dx, estimate[1] + dy)
                    for dx, dy in ((15., 0.), (-15., 0.), (0., 15.), (0., -15.))
                ] + [next_point]
                extra = sum(dist(a, b) for a, b in zip(path, path[1:])) - dist(self.sim.position, next_point)
                if extra <= 250.0:
                    choices.append((extra, channel, estimate))
            if not choices:
                return
            _, channel, estimate = min(choices)
            considered.add(channel)
            if (self._try_clear(estimate, channel)
    ...
    def _route_anchor(self, st: ChannelState) -> Point:
        """Routing node for the scheduler; subclasses may route to action points."""
    ...
    def _two_opt_route(self, channels: list[int]) -> list[int]:
        ...
        if len(channels) < 2:
            return channels
        anchors = {c: self._route_anchor(self.state[c]) for c in channels}
