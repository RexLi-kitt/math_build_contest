...
class OfficialSimulatorClient:
    ...
    def _record(self, kind: str, position, channel, result: str, response: dict) -> None:
        ...
        self.log_path.write_text(
            json.dumps([a.__dict__ for a in self.actions], ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
    ...
    def measure(self, position, channel: int):
        ...
        self._record("measure", self.position, channel, result, response)
    ...
    def clear(self, position, channel: int) -> bool:
        ...
        self._record("clear", self.position, channel, result, response)
    ...
    def exit(self) -> None:
        if not self.entered:
            return
        try:
            response = self._post("/exit", self._base(self._next_id("exit")))
            self._record("exit", None, None, response["exit_reason"], response)
            print("已主动退出测试。")
        finally:
            self.entered = False
...
def main() -> int:
    ...
    sim = OfficialSimulatorClient(args.base_url, args.robot_id, Path(args.log))
