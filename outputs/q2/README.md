...
`run_q2_formal_experiments.py` 采用 20 个训练场景与 35 组单纯形网格权重校准，并用独立的 40 个场景进行真实二测回放；每次权重扰动均重新执行全局粗搜与局部加密，而非重用固定候选池。结果写入 `Q2正式权重网格校准.csv`、`Q2正式独立测试策略对比.csv` 与 `Q2正式权重鲁棒性.csv`。

该批结果是**小规模合成离线验证**，不是官方模拟器正式测试。其独立回放显示，当前策略的二测 MEC≤20m 成功率并非 100%，因此论文必须表述为“二测后若 MEC≤20m 才清除，否则进入后续主动测量闭环”，不得写成“二测必然清除”或“已经达到正式最优”。提交前应扩展至每组至少 30 个训练案例，并按官方演练协议另行复现。
