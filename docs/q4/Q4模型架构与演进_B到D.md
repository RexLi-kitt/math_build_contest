...
flowchart LR
    ...
    BP[B+：B站点覆盖优先顺序] --> DO[DO：B/B+顺序门控<br/>历史消融，旧标识D1]
    C[C：29站正常场景优化] --> CP
    CP --> A3[C+3A：C分支目标3方位]
    A3 --> F3[C+3F：C分支距离门]
    F3 --> D[D：两分支距离门]
...
但边界压力从 562.36 上升到 596.49。DO（历史结果旧标识 `D1`）用原点零发现选择 B 顺序，否则选择 B+ 顺序，
得到正常 655.97、压力 562.36。

这条路线的弱点是候选仍只有同一个 27 站集合，能调整的只是发现日程，无法同时降低整条
巡游成本和搜索后定位成本。DO 被保留为历史消融，不再占用 D 的正式命名。
...
| B+ | normal 发现日程更快 | 边界压力约 +34 | 覆盖优先顺序损害边界日程 | DO 用原点门控顺序 |
| DO（旧 D1） | 保住 B 边界并接近 B+ normal | normal 仍约 656 | 仍受同一 27 站集合限制 | 引入 C 路线 |
